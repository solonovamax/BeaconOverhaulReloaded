...
class StatusEffectControllerElement(
    ...
    screen: YACLScreen,
    dim: Dimension<Int>,
) : RegistryEntryBackedDropdownControllerElement<StatusEffect>(controller, screen, dim) {
    override fun drawCurrentEntry(context: DrawContext, x: Int, y: Int, entry: RegistryEntry<StatusEffect>) {
    ...
    override fun getValueText(): Text {
        ...
        return (control as StatusEffectController).option().pendingValue().value().name
    }
}
